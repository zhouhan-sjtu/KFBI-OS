/*=============================================================================
*   
*   Filename : ReacDiffSolver1dN-D.c
*   Creator : Han Zhou
*   Date : 11/22/21
*   Description : 
*
...
makeCorrection(const VectorXd &x, const VectorXb &interior, int n, 
							 ...
							 double kap_i, double kap_e, const double *psi, 
							 VectorXd &f)
{
	double h = x[1] - x[0];
										 ...
										 const VectorXd &v, VectorX6d &v_bdry_data)
{
	...
	for(int k = 0; k < bdry_num; k++){
		...
		//makeQuadraticInterpolation(t, f, xi, g);
		makeCubicInterpolation(t, f, xi, g);
...
//:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
static void 
extractBoundaryDataV(const VectorXd &x, const VectorXb &interior, int n, 
											const VectorXd &bdry_crd, int bdry_num, 
...
computeMatrixVectorProduct(const VectorXd &x, const VectorXb &interior, int n,
													 const VectorXd &bdry_crd, int bdry_num,
													 double kap_i, double kap_e,
													 const double *psi, double *wx_bdry)
{
	int n1 = n + 1;
	double h = x[1] - x[0];
	double h2 = h * h;
	...
	makeCorrection(x, interior, n, bdry_crd, bdry_num, kap_i, kap_e, psi, f);
	...
	extractBoundaryData(x, interior, n, bdry_crd, bdry_num, kap_i, kap_e,
											 psi,  w, w_bdry_data);

	for(int k = 0; k < bdry_num; k++){
		wx_bdry[k] = w_bdry_data[k][1];
	}
...
//:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
void ReacDiffSolver1dN_D(const VectorXd &x, const VectorXb &interior, int n,
...
{
	// this version solves two point BVP subject to Neumann BC,
	// single layer potential is used (Fredholm IE of the 2nd kind)
	// Dirichlet BC for Green's function

	assert(fabs(kap_i - kap_e) < 1.0e-10);
	...
	extractBoundaryDataV(x, interior, n, bdry_crd, bdry_num, 
												kap_i, kap_e, F, ux_bdry, v, v_bdry_data);
	...
	bool status = GMRES(x, interior, n, bdry_crd, bdry_num, kap_i, kap_e, 
											g, psi, bdry_num, max_m, max_itr_num, 1.0E-8, itr_num);
	...
	extractBoundaryData(x, interior, n, bdry_crd, bdry_num, 
											 kap_i, kap_e, psi, w, w_bdry_data);
